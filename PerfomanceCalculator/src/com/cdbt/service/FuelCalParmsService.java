...
public class FuelCalParmsService {
	...
	public String getDistResult(String tow, String pCount, String cargo, String climbFuel, String descFuel,
			...
			String waiFuelflow, boolean b, boolean engUnit) {
		...
		if (CommonUtils.strToDouble(pCount).doubleValue() * CommonUtils.strToDouble(pass[1]).doubleValue()
			...
			double p = Math.floor((CommonUtils.strToDouble(ac[5]).doubleValue() - CommonUtils.strToDouble(cargo).doubleValue())
		...
		double totalFuel = CommonUtils.strToDouble(tow).doubleValue() - CommonUtils.strToDouble(ac[1]).doubleValue()
				- 91.0D * CommonUtils.strToDouble(crewNum).doubleValue() - CommonUtils.strToDouble(cargo).doubleValue()
				- CommonUtils.strToDouble(pass[1]).doubleValue() * CommonUtils.strToDouble(pCount).doubleValue();

		double altFuel = dWaiFuelflow.doubleValue() * CommonUtils.minToHr(fr[1]).doubleValue() + dAltClimbFuel.doubleValue()
				+ dAltDescFuel.doubleValue()
				+ (CommonUtils.strToDouble(altDist).doubleValue() * 1.852D - dAltClimbDist.doubleValue()
						- dAltDescDist.doubleValue()) / dAltTAS.doubleValue() * dAltFuelFlow.doubleValue();

		double altTime = dAltClimbTime.doubleValue() + dAltDescTime.doubleValue()
				+ (CommonUtils.strToDouble(altDist).doubleValue() * 1.852D - dAltClimbDist.doubleValue()
						- dAltDescDist.doubleValue()) / dAltTAS.doubleValue() * 60.0D;
		...
		double percent = CommonUtils.strToDouble(pCount).doubleValue() * 100.0D / CommonUtils.strToDouble(ac[3]).doubleValue();
		...
		"  距离相关结果：\n        主航程距离：" + CommonUtils.doubleToStr(Double.valueOf(totalDist), Integer.valueOf(2))
				...
				+ "km\n  时间相关结果：\n        主航程耗时：" + CommonUtils.doubleToStr(Double.valueOf(totalTime), Integer.valueOf(2))
				+ "min\n        巡航时间：" + CommonUtils.doubleToStr(Double.valueOf(cruiseTime), Integer.valueOf(2))
				+ "min\n        备降时间：" + CommonUtils.doubleToStr(Double.valueOf(altTime), Integer.valueOf(2))
				+ "min\n,  燃油相关结果：\n        总燃油量：" + CommonUtils.doubleToStr(Double.valueOf(totalFuel), Integer.valueOf(2))
				+ "kg\n        备份油量：" + CommonUtils.doubleToStr(Double.valueOf(altFuel), Integer.valueOf(2))
				+ "kg\n        主航程用油：" + CommonUtils.doubleToStr(Double.valueOf(totalFuelComsumtion), Integer.valueOf(2))
				+ "kg\n        巡航用油：" + CommonUtils.doubleToStr(Double.valueOf(cruiseFuel), Integer.valueOf(2))
				+ "kg\n  其他结果：\n        客座率：" + CommonUtils.doubleToStr(Double.valueOf(percent), Integer.valueOf(1)) + "%";
	}
	...
	public String getDistInputInfo(String tow, String pCount, String cargo, String climbFuel, String descFuel,
			...
			String cruiseFuelFlow, String altClimbFuel, String altDescFuel, String altClimbDist, String altDescDist,
			String altClimbTime, String altDescTime, String altTAS, String altFuelflow, String altDist,
			String waiFuelflow, String yunying, String flightRule, boolean b) {
		if (flightRule != null) {
	...
	public boolean addAC(String ac, String bow, String mtow, String seat, String maxFuel, String maxPayload,
		...
		if ((CommonUtils.ifStringIsDigital(bow)) && (CommonUtils.ifStringIsDigital(mtow)) && (CommonUtils.ifStringIsDigital(seat))
				&& (CommonUtils.ifStringIsDigital(maxFuel)) && (CommonUtils.ifStringIsDigital(maxPayload))
				&& (CommonUtils.ifStringIsDigital(maxTotalload)) && (CommonUtils.ifStringIsDigital(cargo))
				&& (CommonUtils.ifStringIsDigital(alt))) {
			arr.add(ac);
			arr.add(bow);
			arr.add(mtow);
			arr.add(seat);
	...
	public String getWeightResult(String dist, String pCount, String cargo, String climbFuel, String descFuel,
		...
		double cruiseDist = CommonUtils.strToDouble(dist).doubleValue() - dClimbDist.doubleValue() - dDescDist.doubleValue();
